...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      if (status) {
        filteredCourses = filteredCourses.filter(course => {
          // Handle both mock data (published) and real data (status)
          if ('status' in course) {
            return course.status === status.toUpperCase();
          } else if ('published' in course) {
            return status.toUpperCase() === 'PUBLISHED' ? course.published : !course.published;
          }
          return true;
        });
      }
