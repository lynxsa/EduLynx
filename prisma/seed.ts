...
async function main() {
  ...
  const teacherFirstNames = ['Sipho', 'Naledi', 'Teboho', 'Lerato', 'Thabo', 'Mpho', 'Anele', 'Nomsa', 'Tumelo', 'Karabo', 'Sibusiso', 'Gugu', 'Tshepo', 'Bongani', 'Nokuthula', 'Jabulani', 'Thandeka', 'Siyabonga', 'Zanele', 'Ayanda', 'Mandla', 'Nomfundo', 'Thandi', 'Sizwe', 'Lindiwe', 'Nkosana', 'Precious', 'Themba', 'Busisiwe', 'Vusi'];
  const teacherLastNames = ['Dlamini', 'Mokgobu', 'Nkosi', 'Mahlangu', 'Mokoena', 'Radebe', 'Zulu', 'Mbatha', 'Mkhize', 'Khoza', 'Mthembu', 'Ngcobo', 'Mabaso', 'Molefe', 'Mofokeng', 'Motsoeneng', 'Mahlatsi', 'Molekwa', 'Mabena', 'Mokwena', 'Motaung', 'Mthethwa', 'Nkomo', 'Sibeko', 'Khumalo', 'Ndlovu', 'Mthimkhulu', 'Mofokeng', 'Nkabinde', 'Skosana'];
  const qualifications = ['B.Ed', 'B.Sc + PGCE', 'B.A + PGCE', 'B.Com + PGCE', 'M.Ed', 'M.Sc', 'M.A', 'Honours B.Ed', 'B.Tech Education', 'National Diploma Education'];
  const southAfricanCities = ['Johannesburg', 'Pretoria', 'Sandton', 'Randburg', 'Rosebank', 'Parktown', 'Midrand', 'Centurion', 'Germiston', 'Boksburg', 'Benoni', 'Soweto', 'Alexandra', 'Diepsloot', 'Roodepoort'];
  
  const teachers = []
  let tc = 1
  for (const subj of subjects) {
    for (let i = 0; i < 2; i++, tc++) {
      const fn = rand(teacherFirstNames), ln = rand(teacherLastNames)
      ...
      const t = await prisma.teacher.upsert({
        ...
        create: {
          ...
          sex: gender,
          birthday: new Date(1975 + Math.floor(Math.random() * 20), Math.floor(Math.random() * 12), Math.floor(Math.random() * 28) + 1),
  ...
  const parentFirstNames = ['Mary', 'John', 'Thandi', 'Sipho', 'Nomsa', 'Peter', 'Lindiwe', 'Kabelo', 'Zanele', 'Sibusiso', 'Naledi', 'Karabo', 'Bongani', 'Gugu', 'Tshepo', 'Jabulani', 'Thandeka', 'Siyabonga', 'Ayanda', 'Mandla', 'Nomfundo', 'Lucas', 'Beauty', 'David', 'Grace', 'Simon', 'Faith', 'Michael', 'Joy', 'Patrick'];
  const parentLastNames = ['Nkosi', 'Mokoena', 'Dlamini', 'Mahlangu', 'Mabena', 'Molefe', 'Mofokeng', 'Motsoeneng', 'Mahlatsi', 'Molekwa', 'Mabaso', 'Ngcobo', 'Mthembu', 'Khoza', 'Mbatha', 'Zulu', 'Radebe', 'Mkhize', 'Mokwena', 'Motaung', 'Mthethwa', 'Nkomo', 'Sibeko', 'Khumalo', 'Ndlovu', 'Mthimkhulu', 'Nkabinde', 'Skosana', 'Sithole', 'Maseko'];
  const occupations = ['Engineer', 'Teacher', 'Nurse', 'Doctor', 'Lawyer', 'Accountant', 'Manager', 'Administrator', 'Consultant', 'Technician', 'Sales Representative', 'Business Owner', 'Social Worker', 'Pharmacist', 'IT Specialist', 'Marketing Manager', 'Project Manager', 'Human Resources', 'Financial Advisor', 'Government Official'];
  const employers = ['Eskom', 'Sasol', 'Standard Bank', 'FNB', 'Nedbank', 'MTN', 'Vodacom', 'Cell C', 'Discovery', 'Old Mutual', 'Sanlam', 'Anglo American', 'BHP Billiton', 'Department of Education', 'Department of Health', 'City of Johannesburg', 'Gauteng Provincial Government', 'University of the Witwatersrand', 'University of Johannesburg', 'CSIR'];
  const relationships = ['Mother', 'Father', 'Guardian', 'Stepmother', 'Stepfather', 'Grandmother', 'Grandfather', 'Aunt', 'Uncle'];
  
  const parents = [];
  for (let i = 1; i <= 40; i++) {
    const fn = rand(parentFirstNames), ln = rand(parentLastNames);
    const gender = rand([UserSex.MALE, UserSex.FEMALE]);
    const city = rand(southAfricanCities);
    ...
    parents.push(await prisma.parent.upsert({
  ...
  // 7) STUDENTS (200+)
  const studentFirstNames = ['Lebo', 'Sizwe', 'Naledi', 'Thabo', 'Ayanda', 'Kagiso', 'Lerato', 'Sipho', 'Zanele', 'Karabo', 'Tshepo', 'Bongani', 'Gugu', 'Jabulani', 'Thandeka', 'Siyabonga', 'Nomsa', 'Mpho', 'Anele', 'Tumelo'];
  const studentLastNames = parentLastNames;
  for (let i = 1; i <= 200; i++) {
    const fn = rand(studentFirstNames), ln = rand(studentLastNames);
    ...
    students.push(await prisma.student.upsert({
      ...
      create: {
        ...
        birthday: new Date(2007 + Math.floor(Math.random() * 5), Math.floor(Math.random() * 12), Math.floor(Math.random() * 28) + 1),
        parentId: parent.id,
        classId: cls.id,
        gradeId: cls.gradeId,
        schoolId: school.id,
        ...
        extracurriculars: rand(['Soccer', 'Chess', 'Choir', 'Debate', 'Science Club', 'Netball', 'Rugby', 'Drama']),
  ...
  const financeTitles = ['Tuition', 'Book Purchases', 'Sports Equipment', 'Sponsorship', 'Facility Maintenance', 'Uniforms', 'Transport', 'Catering', 'Donations', 'Events'];
  for (let i = 1; i <= 100; i++) {
    await prisma.financeEntry.create({
      data: {
        ...
        category: rand(['School Fees', 'Books', 'Sports', 'Sponsorship', 'Maintenance', 'Uniforms', 'Transport', 'Catering', 'Donations', 'Events']),
    ...
    for (let qn = 1; qn <= 3; qn++) {
      ...
      const question = await prisma.question.create({
        data: {
          ...
          type: qType,
          answer: qType === 'MULTIPLE_CHOICE' ? 'A' : qType === 'TRUE_FALSE' ? 'True' : 'Sample answer',
        }
      })
      ...
      for (const q of questions) {
        await prisma.answer.create({
          data: {
            ...
            questionId: q.id,
            response: q.type === 'MULTIPLE_CHOICE' ? 'A' : q.type === 'TRUE_FALSE' ? 'True' : 'Sample answer',
            isCorrect: true,
          }
        })
      }
