...
const TeacherPage = () => {
  ...
  const genderPieData = [
    { id: 'Male', label: 'Male', value: students.filter(s => s.gender === 'Male').length, color: '#2563eb' },
    { id: 'Female', label: 'Female', value: students.filter(s => s.gender === 'Female').length, color: '#f472b6' },
  ];
        ...
        <div className="col-span-1 md:col-span-2 lg:col-span-4">
          <DashboardCard title="Upcoming Lessons" value={''}>
            <ul className="bg-white rounded shadow p-4">
              {(upcomingLessons || []).length === 0 && <li className="text-gray-400">No upcoming lessons.</li>}
              {(upcomingLessons || []).map((lesson, idx) => (
                <li key={lesson.id || idx} className="mb-2">
                  <span className="font-semibold">{lesson.name}</span> &mdash; {lesson.class?.name} ({lesson.subject?.name})<br />
                  <span className="text-xs text-gray-500">{lesson.startTime ? new Date(lesson.startTime).toLocaleString() : ''}</span>
                </li>
              ))}
            </ul>
          </DashboardCard>
