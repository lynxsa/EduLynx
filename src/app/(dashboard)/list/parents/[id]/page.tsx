...
async function getParent(id: string) {
  const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || ''}/api/parents/${id}`, { cache: 'no-store' });
      ...
      <div className="flex flex-col md:flex-row gap-8 items-center md:items-start bg-white rounded-xl shadow-lg p-6">
        ...
        <div className="flex-1 w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-700">
            ...
            <div><span className="font-semibold">Sex:</span> {parent.sex || 'N/A'}</div>
            <div><span className="font-semibold">Joined:</span> {parent.createdAt ? new Date(parent.createdAt).toLocaleDateString() : 'N/A'}</div>
          </div>
        </div>
      </div>
      <div className="mt-10">
        ...
        {Array.isArray(parent.children) && parent.children.length > 0 ? (
          <div className="space-y-8">
            {parent.children.map((childId: string) => (
              <Suspense key={childId} fallback={<div className="text-gray-500">Loading child info...</div>}>
...
async function getStudent(studentId: string) {
  const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || ''}/api/students/${studentId}`, { cache: 'no-store' });
  if (!res.ok) return null;
  return res.json();
}
