...
export default function ProfLynxAssistant() {
  ...
  const [messages, setMessages] = useState([
    {
      role: 'assistant',
      content: 'Hello! I am Prof. Lynx, your AI assistant. How can I help you today?',
    },
  ]);
  ...
  async function sendMessage() {
    ...
    try {
      ...
      setMessages(msgs => [
        ...msgs,
        { role: 'assistant', content: 'Sorry, Prof. Lynx could not answer right now.' },
      ]);
    } finally {
      setLoading(false);
    }
            ...
            {(messages || []).map((msg, i) => (
              <div key={i} className={msg.role === 'assistant' ? 'text-left' : 'text-right'}>
                <span
                  className={
                    msg.role === 'assistant'
                      ? 'bg-blue-100 text-blue-900 px-2 py-1 rounded'
