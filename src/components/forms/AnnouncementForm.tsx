...
const AnnouncementForm = ({ type, data, onSuccess }: {
  ...
  const { register, handleSubmit, reset, formState: { errors } } = useForm<AnnouncementFormInputs>({
    defaultValues: data || {},
  });
  ...
  return (
    <form className="flex flex-col gap-6" onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <InputField label="Description" name="description" register={register} defaultValue={data?.description || ""} error={errors.description} />
        <InputField label="Date" name="date" type="date" register={register} defaultValue={data?.date?.slice(0,10) || ""} error={errors.date} />
        <div className="flex flex-col gap-2 w-full md:w-1/4">
          <label htmlFor="classId" className="text-xs text-gray-500">Class (optional)</label>
