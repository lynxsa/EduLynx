import { useTheme } from '@/styles/theme';
import Image from 'next/image';
import { useCallback, useEffect, useState } from 'react';

// Base64 encoded logo as ultimate fallback
const LOGO_BASE64 =
  'data:image/png;base64,';

// Logo paths for light and dark themes
const LIGHT_LOGO_PATHS = [
  '/logo.png',
  '/edulynx-logo.png',
  '/lynx-logo.png',
  '/brand-logo.png',
  '/assets/logos/edulynx-main.png',
  '/assets/logos/edulynx-primary.png',
  '/assets/logos/edulynx-fallback.png',
];

const DARK_LOGO_PATHS = [
  '/assets/logos/logo-w.png',
  '/assets/logos/edulynx-primary.png',
  '/assets/logos/edulynx-fallback.png',
  '/logo.png',
  '/edulynx-logo.png',
  '/lynx-logo.png',
  '/brand-logo.png',
  '/assets/logos/edulynx-main.png',
];

interface EduLynxLogoProps {
  width?: number;
  height?: number;
  className?: string;
  alt?: string;
  priority?: boolean;
  forceWhiteLogo?: boolean; // Force white logo regardless of theme
}

export default function EduLynxLogo({
  width = 64,
  height = 64,
  className = 'object-contain',
  alt = 'EduLynx Logo',
  priority = true,
  forceWhiteLogo = false,
}: EduLynxLogoProps) {
  const { actualTheme } = useTheme();
  const [currentPathIndex, setCurrentPathIndex] = useState(0);
  const [useBase64, setUseBase64] = useState(false);
  const [hasLoaded, setHasLoaded] = useState(false);

  // Choose the appropriate logo paths based on theme or force white logo
  const LOGO_PATHS = forceWhiteLogo || actualTheme === 'dark' ? DARK_LOGO_PATHS : LIGHT_LOGO_PATHS;

  const handleError = useCallback(() => {
    console.warn(`Logo failed to load from: ${LOGO_PATHS[currentPathIndex]}`);

    if (currentPathIndex < LOGO_PATHS.length - 1) {
      // Try next logo path
      setCurrentPathIndex(prev => prev + 1);
      console.log(`Trying fallback logo: ${LOGO_PATHS[currentPathIndex + 1]}`);
    } else {
      // Use base64 fallback as last resort
      console.log('Using base64 fallback logo');
      setUseBase64(true);
    }
  }, [currentPathIndex, LOGO_PATHS]);

  const handleLoad = useCallback(() => {
    console.log(
      `✅ Logo loaded successfully from: ${useBase64 ? 'base64 fallback' : LOGO_PATHS[currentPathIndex]}`
    );
    setHasLoaded(true);
  }, [currentPathIndex, useBase64, LOGO_PATHS]);

  // Reset to first logo path when theme changes
  const resetPathIndex = useCallback(() => {
    setCurrentPathIndex(0);
    setUseBase64(false);
    setHasLoaded(false);
  }, []);

  // Effect to handle theme changes
  useEffect(() => {
    resetPathIndex();
  }, [actualTheme, forceWhiteLogo, resetPathIndex]);

  // If we're using base64, render base64 image
  if (useBase64) {
    return (
      <img
        src={LOGO_BASE64}
        alt={alt}
        width={width}
        height={height}
        className={className}
        onLoad={handleLoad}
        onError={() => console.error('❌ Even base64 logo failed to load')}
        style={{
          maxWidth: `${width}px`,
          maxHeight: `${height}px`,
          opacity: hasLoaded ? 1 : 0.8,
        }}
      />
    );
  }

  // Otherwise use Next.js Image with current fallback path
  return (
    <Image
      src={LOGO_PATHS[currentPathIndex]}
      alt={alt}
      width={width}
      height={height}
      className={className}
      priority={priority}
      onLoad={handleLoad}
      onError={handleError}
      style={{
        opacity: hasLoaded ? 1 : 0.8,
      }}
    />
  );
}
